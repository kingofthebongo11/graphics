...
def create_curve_box(input_frame, i, checkbox_var, saved_data):
    ...
    combo_curve_typeX_type.bind(
        "<<ComboboxSelected>>",
        lambda e: saved_data[i - 1].update({'curve_typeX_type': combo_curve_typeX_type.get()})
    )
    ...
    combo_curve_typeY_type.bind(
        "<<ComboboxSelected>>",
        lambda e: saved_data[i - 1].update({'curve_typeY_type': combo_curve_typeY_type.get()})
    )
