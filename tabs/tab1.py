...
def create_tab1(notebook: ttk.Notebook) -> None:
    ...
    def show_usage() -> None:
        ...
        text = (
            "1. В верхней части вкладки выберите язык интерфейса — он влияет на подписи осей, заголовки, легенду и элементы управления.\n"
            "2. Настройте заголовок графика: выберите готовый вариант, пункт «Другое» для собственного текста или «Нет», чтобы скрыть подпись.\n"
            "3. Для осей X и Y выберите физическую величину. Пункт «Другое» открывает поле для своего названия, «Нет» убирает подписи. В списке размерностей выберите единицы измерения или задайте их вручную.\n"
            "4. Отметьте флажок «Легенда», если нужны подписи кривых: в появившемся списке можно выбрать готовый заголовок легенды или пункт «Другое» и ввести текст.\n"
            "5. Укажите количество кривых и загрузите данные для каждой. Поддерживаются текстовые файлы (*.txt, *.dat), таблицы (*.xlsx, *.xlsm, *.csv) с выбором ориентации, диапазонов и смещений, а также файлы кривых и результаты частотного анализа LS-DYNA.\n"
            "6. При необходимости используйте дополнительные параметры чтения: для частотного анализа доступны выбор направления и параметра, а при обработке LS-DYNA служебные строки проверяются автоматически.\n"
            "7. Нажмите «Построить график». В блоке «Настройки осей» появятся актуальные авто-пределы; чтобы зафиксировать собственные значения, заполните поля «вручную» и нажмите «Применить».\n"
            "8. Для добавления отметок включите «Режим отметок», выберите тип подписи (X, Y, (X,Y) или «Свой текст»), настройте привязку к нужной кривой, форму, размер и цвет маркера. Цвет можно выбрать из палитры или через диалог, доступны кнопки отмены, повтора и очистки всех отметок.\n"
            "9. После построения графика откроется редактор линий: в нём можно менять цвета, стили и толщины, применять палитры и ограничивать диапазон данных ползунками. Флажок «Фиксировать оси» закрепляет текущие пределы.\n"
            "10. Для сохранения изображения введите имя файла, выберите формат (png, jpg, svg, pdf) и нажмите «Сохранить».\n"
            "11. Кнопки «Сохранить проект» и «Загрузить проект» позволяют сохранить всю конфигурацию вкладки в JSON и восстановить её позднее."
        )
