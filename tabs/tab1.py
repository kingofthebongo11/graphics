...
def create_tab1(notebook: ttk.Notebook) -> None:
    ...
    def show_usage() -> None:
        ...
        text = (
            "1. Выберите заголовок графика из списка или пункт «Другое» для ввода собственного текста.\n"
            "2. Для осей X и Y выберите величину из списка: вариант «Другое» позволяет ввести своё название, а «Нет» скрывает название и единицы; при необходимости задайте размерность.\n"
            "При выборе «Нет» в комбобоксах заголовка графика и осей подпись не отображается.\n"
            "3. Выберите количество кривых и загрузите данные для каждой; при необходимости активируйте чекбокс «Легенда» и введите подпись кривой. Без включённой легенды подписи не отображаются.\n"
            "4. Поддерживаемые форматы данных: текстовые файлы (*.txt, *.dat) — каждая строка содержит два числа; Excel-файлы (*.xlsx, *.xlsm, *.csv) с двумя столбцами или строками и возможностью указания диапазонов; файлы кривых LS-DYNA; результаты частотного анализа LS-DYNA.\n"
            "5. Дополнительные настройки: для Excel можно менять ориентацию (строки/столбцы), задавать смещения и диапазоны; для частотного анализа выбираются параметр и направление; при чтении LS-DYNA проверяются служебные маркеры и игнорируются строки-комментарии.\n"
            "6. Нажмите «Построить график» для отображения.\n"
            "7. После построения графика откроется редактор, где можно изменять цвет, толщину и стиль линий, "
            "применять цветовые палитры; все изменения сразу видны на графике.\n"
            "8. Для сохранения изображения сначала введите имя файла, затем выберите формат (png, jpg, svg, pdf) и нажмите «Сохранить». "
